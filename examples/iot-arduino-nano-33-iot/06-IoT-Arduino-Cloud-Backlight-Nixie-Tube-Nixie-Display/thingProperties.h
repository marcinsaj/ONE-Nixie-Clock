// If you decide to use the Arduino Cloud Code Editor
// then this file should be omitted.
// Use only with Arduino IDE offline software.

// Code generated by Arduino IoT Cloud.

#include <ArduinoIoTCloud.h>
#include <Arduino_ConnectionHandler.h>

// *********************************************************************************
const char THING_ID[] = "place-here-id-number-of-your-thing-from-iot-arduino-cloud";
// *********************************************************************************

const char SSID[]     = SECRET_SSID;    // Network SSID (name)
const char PASS[]     = SECRET_PASS;    // Network password (use for WPA, or use as key for WEP)

void onNixieTubeChange();
void onNixieDisplayChange();
void onBacklightChange();

bool nixie_Tube;
int nixie_Display;
CloudColoredLight backlight;

void initProperties(){

  ArduinoCloud.setThingId(THING_ID);
  ArduinoCloud.addProperty(nixie_Tube, READWRITE, ON_CHANGE, onNixieTubeChange);
  ArduinoCloud.addProperty(nixie_Display, READWRITE, ON_CHANGE, onNixieDisplayChange);
  ArduinoCloud.addProperty(backlight, READWRITE, ON_CHANGE, onBacklightChange);

}

WiFiConnectionHandler ArduinoIoTPreferredConnection(SSID, PASS);
