...
// Code only for Arduino Nano and Nano Every
//
// This example demonstrates how to display digits nad symbols 
// Fade in/out effect for multisegment tubes has been turned off
// because Arduino Nano default pwm frequency is 490Hz & Nano Every 976Hz  
// and with these values there are the undesirable effect 
// of "singing tube" - audible noise.
// This option could be turned on by uncommenting two lines 
...
// Without PWM fade in/out effect for multisegment tubes
// Arduino Nano Every default pwm frequency is 976kHz 
// and with that value there is the undesirable effect of "singing tube" - audible noise 
void ShowSymbol()
{       
  for(int symbol = 0; symbol < 36; symbol++)
  {
    uint16_t current_symbol = 0;
    uint16_t new_symbol = 0;
    
    for(int animation = 0; animation < 15; animation++)
    {      
      if(animation_nixie_tube[animation] & symbol_nixie_tube[symbol])
      {
        new_symbol = new_symbol | (animation_nixie_tube[animation] & symbol_nixie_tube[symbol]);
        current_symbol = new_symbol;
      }
      else
      {
        if(animation == 14) current_symbol = new_symbol;
        else current_symbol = new_symbol | animation_nixie_tube[animation];
      }
        
      ShiftOutData(current_symbol);
      delay(70);
    } 
